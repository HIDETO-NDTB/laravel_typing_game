                ...
                <div class="card">
                    ...
                    <div class="card-body text-center drill-body">
                        ...
                        <template v-if="isStarted && !isCountDown && !isEnd">
                            ...
                            <span style="font-size: 50px" v-for="(word, index) in problemWords" :class="{'text-primary': index < currentWordNum}">{{word}}</span>
                        </template>

                        <template v-if="isEnd">
                            <p style="font-size: 50px">あなたのスコア</p>
                            <p style="font-size: 80px">{{typingScore}}</p>
                        </template>
                    </div>
                </div>
            </div>
...
export default {
    ...
    computed: {
        ...
        //         problemWords.push(question.question)
        //     })
        //     console.log(problemWords)
        //     return problemWords
        // },
        problemWords: function () {
            return Array.from(this.questions[this.currentProblemNum].question)
        },
        // 問題の解答キーコード配列
        problemKeyCodes: function () {
            if(!Array.from(this.questions[this.currentProblemNum].question).length){
                return null
            }

            // テキストから問題のキーコード配列を生成
            let problemKeyCodes = []
            console.log(Array.from(this.questions[this.currentProblemNum].question))
            Array.from(this.questions[this.currentProblemNum].question).forEach((text) => {
